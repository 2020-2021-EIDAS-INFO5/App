...
![Alt text](./Fig/PolySignlogo.png "Title")
...
We called ou web application `PolySign`.

## How does it work ?
Firstly we lanch Jhipster as below :
...
To build the final jar and optimize the polySign application for production, run:
...
## 2. Keyclock
For authentication we decided to use the keycloak framework with the OpenID connect protocol based on the Oauth2 protocol
...
Firstly, open the link : http://localhost:9080/auth/ 

Then, open the administration console enter the credentials 
```
Username: admin
Password:admin
```
You must now, configure keycloack by importing our configuration json file which contains the roles of the realm in keycloack 

```
realm-export_ .json
```
1. Set Server and Host email :
![Alt text](./Fig/Email.png "Title")

2. Copy the official email in the tab below :

![Alt text](./Fig/editAccount.png "Title")

3. Set client parameters like below :

![Alt text](./Fig/Client.png "Title")

4. You must then configure the standard access to confidential. A secret  code is then shared with the jhipster application :

![Alt text](./Fig/token.png "Title")

5. you have to copy this secret code in the following files: 

```
src/main/ressource/config/application.yml
```

```
src/main/java/com/polytech/polysign/config/Constant.java
```
```
src/main/ressources/config/application.yml
```



##### **NB : All figures and images of this README are in `Fig` Folder 
...
export class UserEntityComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.userEntityService
      ...
      .subscribe(
        (res: HttpResponse<IUserEntity[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IUserEntity): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInUserEntities(): void {
    this.eventSubscriber = this.eventManager.subscribe('userEntityListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IUserEntity[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
